...
class TestNetworkCombiner:
    ...
    def test_combine_by_preferential_attachment(self) -> None:
        """Test if combination works well."""
        n_nodes_to_add = 100
        network = NetworkReader().read_cunha()
        org_network_size = network.number_of_nodes()

        new_network = NetworkCombiner.combine_by_preferential_attachment(
            network, n_nodes_to_add, 2
        )
        assert isinstance(new_network, nx.Graph), "Network wasn't created properly"

        # New network should have 100 new nodes
        assert (
            new_network.number_of_nodes() == org_network_size + n_nodes_to_add
        ), "Number of new nodes is not correct!"

        # Test if the network is following a powerlaw distribution
        # First convert to Networkit
        nk_network = NetworkConverter.nx_to_nk(new_network)
