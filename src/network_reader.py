...
class NetworkReader():
    ...
    def read_montagna_meetings(self):
        ...
        """
        data = pd.read_csv(self.directory+'Montagna_Meetings_Edgelist.csv', sep = '\s+')
        
        graph_obj = nx.from_pandas_edgelist(data, source='Source', target='Target', 
                                                    edge_attr=['Weight'])
        return graph_obj
    ...
    def read_montagna_phone_calls(self):
        ...
        https://github.com/lcucav/networkdisruption
        """
        data = pd.read_csv(self.directory+'Montagna_Phone_Calls_Edgelist.csv', sep = '\s+')
