...
cdef class SimMartVaq:
    ...
    cdef:
        ...
        float ratio_honest
        float ratio_wolf
        float ratio_criminal
        bint criminal_acting
        bint wolf_acting
    ...
    def __init__(
        ...
        assert isinstance(self.network, gt.Graph), "Network should be of type gt."
        assert self.network.vp.state, "Network has no attribute state"
        self.ratio_honest = (
            len(gt.find_vertex(self.network, self.network.vp.state, "h"))
            / self.network.num_vertices()
        )
        self.ratio_wolf = (
            len(gt.find_vertex(self.network, self.network.vp.state, "w"))
            / self.network.num_vertices()
        )
        self.ratio_criminal = (
            len(gt.find_vertex(self.network, self.network.vp.state, "c"))
            / self.network.num_vertices()
        )
    ...
    cpdef investigation_stage(
        self,
        network,
        frozenset group_members,
        int slct_pers,
        str slct_status,
    ):
    ...
    cpdef float conducting_investigation(
        self, frozenset group_members, int slct_pers ,float penalty_score
    ):
    ...
    cpdef (int, int, int, float, float, float) counting_status_proportions(
        self, object network, frozenset group_members,
    ) :
    ...
    cpdef (float, float, float) get_overall_fitness_distribution(
        self, object network, list group_members
    ):
    ...
    cpdef dict slct_pers_n_neighbours(
        self, object network, int n_groups, int network_size
    ):
        ...
        cdef:
            dict communities
            np.ndarray protagonists = np.zeros([n_groups], dtype=DTYPE)
            int protagonist
        communities = {}
    ...
    cpdef object mutation(self, object network, int person):
